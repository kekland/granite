...
const generatedLocation = path.resolve(rootLocation, 'lib', 'spec', 'gen', 'style.gen.dart');

const reference = require(path.resolve(rootLocation, 'references', 'spec', 'v8.json'));
...
const getDartType = (object) => {
  ...
  if (type === 'array') {
    ...
    if (subtype === 'number') {
      if (object.length === 2) return 'Vector2';
      if (object.length === 3) return 'Vector3';
      if (object.length === 4) return 'Vector4';
    }
  ...
  for (const field of fields) {
    ...
    if (hasDefaultValue) {
      ...
      if (type.startsWith('Vector')) {
        defaultValue = `Vector${type.substring(6)}(${defaultValue})`;
      }
      if (type === 'Color') {
...
const generateFromJsonFactory = ($class) => {
  ...
  for (const field of fields) {
    ...
    if (type.startsWith('List<')) {
      ...
      else if (subtype.startsWith('Vector')) {
        const vectorType = subtype.substring(6);
        const className = `Vector${vectorType}`;
        const method = `${className.toLowerCase()}FromJson`;
        _cast = `(json['${rawName}'] as List).map((e) => ${method}(e)).toList()`;
      }
      ...
      const vectorType = type.substring(6);
      const className = `Vector${vectorType}`;
      const method = `${className.toLowerCase()}FromJson`;

      _cast = `${method}(json['${rawName}'] as List)`;
    }
    else if (type === 'Layout') {
...
const generateFieldGetter = ($class) => {
  ...
  const code = [];
  code.push('  @override');
  code.push('  Property getProperty(Symbol field) => switch(field) {');
  for (const field of fields) {
    const { name, type } = field;
    code.push(`    #${name} => ${name} as Property,`)
  }
  code.push(`    _ => throw Exception('Unknown field: \$field'),`);
  code.push('  };');
...
const generateDartCode = () => {
  ...
  code.push('  }')
  code.push('')
  code.push('  Property getProperty(Symbol field);')
  code.push('}')
  code.push('')
