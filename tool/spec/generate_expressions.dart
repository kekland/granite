...
import 'package:granite/spec/expression/annotation.dart';
import 'package:granite/spec/expression/expression_dependency.dart';
...
ExpressionAnnotation _getExpressionAnnotation(Declaration expr) {
  final fragment = expr.declaredFragment as TopLevelFunctionFragment;
  final expressionAnnotation = fragment.metadata.annotations.firstWhereOrNull((v) => v.element is ConstructorElement);
...
String? _getExpressionCustomFromJson(Declaration expr) {
  ...
  return value.getField('customFromJson')?.toFunctionValue2()?.name;
}

List<FormalParameterElement> _getExpressionParameters(FunctionDeclaration decl) {
  final expr = decl.functionExpression;
  final parameters = <FormalParameterElement>[];

  for (final fragment in expr.parameters!.parameterFragments) {
    final parameter = fragment!.element;
...
List<FormalParameterElement> _getExpressionChildren(FunctionDeclaration decl) {
  final parameters = _getExpressionParameters(decl);
  final result = <FormalParameterElement>[];
...
Future<void> main() async {
  ...
  final expressionsDirectory = Directory('${rootDirectory.path}/lib/spec/expression/definitions');
  final outputFile = File('${rootDirectory.path}/lib/spec/gen/expressions.gen.dart');
  final referenceFile = File('${rootDirectory.path}/references/spec/v8.json');
  ...
  code.add('// Generated by tool/style/generate_expressions.dart');

  code.add('');
  code.add('import \'../spec.dart\';');
  code.add('import \'../expression/definitions/_definitions.dart\';');
  code.add('');
